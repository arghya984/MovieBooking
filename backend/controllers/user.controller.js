...
module.exports.signUp = async (req, res) => {
    try {
        ...
        const lastUser = await Users.findOne().sort({ userid: -1 }); // Find the last user
        let user_id = 1; // Default user_id if no user exists yet

        if (lastUser) {
            user_id = lastUser.userid + 1; // Increment user_id if a user exists
        }
        ...
        console.log(user);
        await user.save();
        res.status(200).json({ message: "user successfully created", user_id }); // Send user_id in the response
    } catch (error) {
